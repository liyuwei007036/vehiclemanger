...
public class EveChooseModelActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_eve_chooese_model);
        initToolBar();
        initView();
        initData();
        getModelData();
    ...
    private void getModelData() {
        ...
        data.put("seriesId", series_id);
        OkHttpUtils.getInstance().PostWithJson("/getModelsBySeriesId", data.toJSONString(), new OkHttpUtils.ResultCallBackListener() {
            @Override
            ...
            public void onSuccess(String url, String result_str) {
                ...
                if (res.getBoolean("valid")) {
                    JSONObject list = res.getJSONObject("data");
                    JSONArray models = new JSONArray();
                    for (String key : list.keySet()) {
                        JSONArray a = list.getJSONArray(key);
                        for (int i = 0; i < a.size(); i++) {
                            JSONObject object = a.getJSONObject(i);
                            String name = key + object.getString("name");
                            object.put("name", name);
                            models.add(object);
                        }
                    }
                    recycler.setAdapter(new ModelAdapter(getModels(models), model_id));
                    recycler.addItemDecoration(new DividerItemDecoration(EveChooseModelActivity.this, DividerItemDecoration.VERTICAL));
                }
            }
